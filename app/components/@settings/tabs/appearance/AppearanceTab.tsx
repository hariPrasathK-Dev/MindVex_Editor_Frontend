...
export default function AppearanceTab() {
    ...
    const savePrefs = useCallback(
        (updates: Partial<{ theme: ThemeMode; fontSize: FontSize; accentColor: AccentColor; animationsEnabled: boolean; compactMode: boolean }>) => {
            const newPrefs = {
                theme,
                fontSize,
                accentColor,
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="grid grid-cols-3 gap-3">
                    {([
                        ...
                        >
                            <div className={classNames(item.icon, 'w-6 h-6 mb-2', theme === item.id ? 'text-purple-500' : 'text-gray-400')} />
                            <div className={classNames('text-sm font-medium', theme === item.id ? 'text-purple-700 dark:text-purple-300' : 'text-gray-900 dark:text-white')}>
                                {item.label}
                            </div>
                    ...
                    {ACCENT_COLORS.map((color) => (
                        <button
                            ...
                            className={classNames(
                                ...
                                accentColor === color.id ? 'ring-2 ring-offset-2 ring-offset-white dark:ring-offset-[#0A0A0A] scale-110' : 'hover:scale-105',
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                ...
                <div className="grid grid-cols-3 gap-3">
                    {([
                        ...
                        >
                            <span
                                className={classNames('font-mono', fontSize === item.id ? 'text-purple-600 dark:text-purple-400' : 'text-gray-600 dark:text-gray-300')}
                                style={{ fontSize: item.size }}
                            >
                                Aa
                            </span>
                            <div className={classNames('text-xs mt-1', fontSize === item.id ? 'text-purple-600 dark:text-purple-400' : 'text-gray-500')}>
