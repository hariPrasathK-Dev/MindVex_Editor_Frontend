...
const VULN_TYPES = [
  ...
  },
  {
    ...
    cwe: 'CWE-284',
    remediation: 'Implement strict access control checks on this endpoint.',
  },
  {
    type: 'Hardcoded Credentials',
    cwe: 'CWE-798',
    remediation: 'Move secrets to an environment vault or secrets manager.',
  },
  {
    type: 'Path Traversal',
    cwe: 'CWE-22',
    remediation: 'Validate and sanitize file path inputs against a strictly allowed list.',
  },
  {
    type: 'Unrestricted File Upload',
    cwe: 'CWE-434',
    remediation: 'Verify file signatures and restrict executable MIME types.',
  },
];
...
export function CodeHealthHeatmap() {
  ...
  const metrics = useMemo(() => {
    ...
    fileEntries.forEach((filePath) => {
      ...
      if (filePath.endsWith('.tsx') || filePath.endsWith('.java')) {
        coverage = Math.floor(40 + randomVal * 60);
      }
      ...
      for (let i = 0; i < vulnCount; i++) {
        ...
        if (severityObj > 0.9) {
          severity = 'critical';
        } else if (severityObj > 0.6) {
          severity = 'high';
        } else if (severityObj > 0.3) {
          severity = 'medium';
        }
  ...
  const displayData = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (sortBy === 'name') {
        ...
        comparison = a.loc - b.loc;
      } else {
        if (viewMode === 'coverage') {
          comparison = a.coverage - b.coverage;
        } else if (viewMode === 'debt') {
          comparison = b.techDebtScore - a.techDebtScore;
        } // Higher debt first
        else {
          const getSevScore = (m: FileMetric) =>
            m.vulnerabilities.reduce(
              (sum, v) =>
                sum +
                (v.severity === 'critical' ? 1000 : v.severity === 'high' ? 100 : v.severity === 'medium' ? 10 : 1),
              0,
            );
  ...
  const criticalVuls = metrics.reduce(
    (s, m) => s + m.vulnerabilities.filter((v) => v.severity === 'critical').length,
    0,
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#050505] text-white">
          ...
          <Card className="bg-black/40 border-white/5 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-black text-indigo-400">
                {avgDebt} <span className="text-sm font-medium text-gray-500">/ 100</span>
              </div>
            </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          className={`flex flex-col border-r border-white/5 p-6 overflow-y-auto transition-all duration-300 ${selectedFile ? 'w-1/2' : 'w-full'}`}
        >
          <div className="flex items-center justify-between mb-4 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-[#111] p-1 rounded-xl border border-white/10">
              ...
              <button
                ...
                className="p-1 hover:text-teal-400 text-gray-500 transition-colors"
              >
            ...
            {displayData.map((file, idx) => {
              ...
              if (viewMode === 'vulnerabilities') {
                ...
                if (isCrit) {
                  ...
                  severityBorder = 'border-red-500/40 shadow-[0_4px_20px_rgba(239,68,68,0.15)] ring-1 ring-red-500/20';
                  bgGlow = 'bg-red-500/5';
                } else if (isHigh) {
                  ...
                  severityBorder = 'border-yellow-500/20';
                ...
                if (file.coverage < 40) {
                  ...
                  severityBorder = 'border-emerald-500/20';
                }
              } else {
                if (file.techDebtScore > 75) {
                  scoreColor = 'text-indigo-400';
                  severityBorder = 'border-indigo-500/40';
                  bgGlow = 'bg-indigo-500/5';
                } else if (file.techDebtScore > 40) {
                  scoreColor = 'text-blue-400';
                  severityBorder = 'border-blue-500/20';
                } else {
                    ...
                    {viewMode === 'vulnerabilities' && file.vulnerabilities.length > 0 && (
                      <div className="flex -space-x-1">
                        {file.vulnerabilities.slice(0, 3).map((v, i) => (
                          <div
                            key={i}
                            className={`w-3 h-3 rounded-full border border-[#111] ${v.severity === 'critical' ? 'bg-red-500' : v.severity === 'high' ? 'bg-orange-500' : 'bg-yellow-500'}`}
                          ></div>
                        ))}
                      </div>
                    )}
            ...
            <div className="p-6 border-b border-white/5 sticky top-0 bg-[#0a0a0a]/80 backdrop-blur-xl z-30">
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  className="p-2 text-gray-500 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors"
                >
              ...
              <div className="flex gap-2 mt-6">
                <Card className="flex-1 bg-black/40 border-white/5 p-3">
                  ...
                  <div
                    className={`text-xl font-black ${activeFileData.coverage < 50 ? 'text-red-400' : 'text-emerald-400'}`}
                  >
                    {activeFileData.coverage}%
                  </div>
                </Card>
                <Card className="flex-1 bg-black/40 border-white/5 p-3">
                  ...
                  <div
                    className={`text-xl font-black ${activeFileData.vulnerabilities.length > 0 ? 'text-red-400' : 'text-gray-400'}`}
                  >
                    {activeFileData.vulnerabilities.length}
                  </div>
                </Card>
                <Card className="flex-1 bg-black/40 border-white/5 p-3">
                ...
                {activeFileData.vulnerabilities.length === 0 ? (
                  <div className="p-8 border border-emerald-500/20 bg-emerald-500/5 rounded-2xl flex flex-col items-center text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-2 max-w-xs">
                      CodeNexus AI scanning found no known CVEs, injections, or static analysis warnings in this file.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {activeFileData.vulnerabilities.map((vuln, i) => (
                      <div key={i} className="border border-white/10 bg-black/30 rounded-xl overflow-hidden group">
                        <div
                          className={`p-4 border-l-4 ${vuln.severity === 'critical' ? 'border-red-500 bg-red-500/5' : vuln.severity === 'high' ? 'border-orange-500 bg-orange-500/5' : 'border-yellow-500 bg-yellow-500/5'}`}
                        >
                          <div className="flex items-start justify-between">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <Badge
                                  variant="outline"
                                  className={`font-mono text-[9px] uppercase px-1.5 py-0 ${vuln.severity === 'critical' ? 'border-red-500/50 text-red-400' : vuln.severity === 'high' ? 'border-orange-500/50 text-orange-400' : 'border-yellow-500/50 text-yellow-400'}`}
                                >
                                  {vuln.severity}
                                </Badge>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      ...
                      <div className="flex items-end gap-2 mb-2">
                        <span
                          className={`text-4xl font-black ${activeFileData.techDebtScore > 70 ? 'text-red-400' : activeFileData.techDebtScore > 40 ? 'text-orange-400' : 'text-emerald-400'}`}
                        >
                          {activeFileData.techDebtScore}
                        </span>
                        <span className="text-sm text-gray-500 font-bold mb-1">/ 100</span>
                      </div>
                      <p className="text-xs text-gray-400 leading-relaxed">
                        {activeFileData.techDebtScore > 70
                          ? 'High refactoring priority. File exceeds cognitive complexity limits.'
                          : 'Maintainable structure. Keep cognitive loads low.'}
                      </p>
                    </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="font-mono text-orange-400">
                            {Math.floor(seededRandom(activeFileData.path + 'churn') * 100)}%
                          </span>
                        </div>
                      </div>
                    </div>
...
function AiFixButton({ file, vuln, repoUrl }: { file: string; vuln: any; repoUrl: string }) {
  ...
  const handleFix = async () => {
    ...
    try {
      ...
      if (!fileContent) {
        throw new Error('File content not available locally.');
      }

      if (!repoUrl) {
        throw new Error('No repository active.');
      }
...
${fileContent}`;
      ...
      const fixedCode = response.reply
        .replace(/^```[a-z]*\n/i, '')
        .replace(/\n```$/i, '')
        .trim();
      ...
      if (!isGithub) {
        ...
        // Still marked as done so user can see the fix
        return;
      }
      ...
      if (!createBranchRes.ok) {
        throw createBranchRes;
      }

      // 4. Get current file SHA
      const relativeGitHubPath = extractRelativePath(file);
      const fileRes = await fetch(`${proxyBase}/contents/${relativeGitHubPath}?ref=${defaultBranch}`, {
      ...
      if (!fileData || !fileData.sha) {
        throw new Error('Could not find file on GitHub remote.');
      }
        ...
        try {
          ...
          if (e.status === 404 || e.status === 403) {
            if (repoUrl.includes('github.com')) {
              toast.warning(
                'Auth Hint: You might not have write permissions to this repository. Ask the owner for collaborator access.',
                { autoClose: 10000 },
              );
            }
          }
